...
test.describe('/settings 페이지 (비로그인 접근 가능)', () => {
  ...
  test('테마 설정이 표시되어야 함', async ({ page }) => {
    ...
    // 메인 컨텐츠 영역의 테마 버튼들 확인
    const mainContent = page.locator('#main-content');
    await expect(mainContent.getByRole('button', { name: '라이트 모드' })).toBeVisible();
    await expect(mainContent.getByRole('button', { name: '다크 모드' })).toBeVisible();
    await expect(mainContent.getByRole('button', { name: '시스템 설정 따르기' })).toBeVisible();
  });
  ...
  test('다크 모드 클릭 시 테마가 변경되어야 함', async ({ page }) => {
    ...
    // 메인 컨텐츠의 다크 모드 버튼 클릭
    const mainContent = page.locator('#main-content');
    await mainContent.getByRole('button', { name: '다크 모드' }).click();
...
test.describe('/mypage 페이지 (로그인 필수)', () => {
  ...
  test.describe('로그인 상태', () => {
    ...
    test('프로필 관리 메뉴가 있어야 함', async ({ page }) => {
      await page.goto('/mypage');
      const mainContent = page.locator('#main-content');
      await expect(mainContent.getByText('프로필 수정')).toBeVisible();
      await expect(mainContent.getByRole('link', { name: /비밀번호 변경/ })).toBeVisible();
      await expect(mainContent.getByText('알림 설정')).toBeVisible();
    });

    test('내 활동 메뉴가 있어야 함', async ({ page }) => {
      await page.goto('/mypage');
      const mainContent = page.locator('#main-content');
      await expect(mainContent.getByText('내 게시글')).toBeVisible();
      await expect(mainContent.getByText('내 댓글')).toBeVisible();
      await expect(mainContent.getByText('좋아요한 글')).toBeVisible();
      await expect(mainContent.getByRole('link', { name: /북마크/ })).toBeVisible();
    });

    test('로그아웃 버튼 클릭 시 확인 다이얼로그가 표시되어야 함', async ({ page }) => {
      await page.goto('/mypage');
      const mainContent = page.locator('#main-content');
      await mainContent.getByRole('button', { name: /로그아웃/i }).click();
    ...
    test('로그아웃 확인 시 토큰이 삭제되어야 함', async ({ page }) => {
      ...
      // 다이얼로그의 로그아웃 확인 버튼 클릭 (danger 버튼)
      const modal = page.locator('.fixed.inset-0');
      await modal.getByRole('button', { name: '로그아웃' }).click();
